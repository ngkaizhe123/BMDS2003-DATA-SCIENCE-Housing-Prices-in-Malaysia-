...
def initialize_form_memory(keys):
    """Sets default values if the user is opening the app for the first time."""
...
def render_interactive_map(keys, form_key, state_area_lookup):
    ...
    # Draw Map Locked to Malaysia Boundaries
    m = folium.Map(
        location=[center_lat, center_lon],
        zoom_start=zoom_level, min_zoom=5, max_bounds=True,
    ...
    # Map Click Memory (Prevents infinite reload loops)
...
def render_input_form(form_key):
    ...
    st.info(
        "💡 **Tip:** Use a preset below to auto-fill the form, or enter/select your own custom details."
    )
    # 2. Reserve Empty Space for Top Dropdown (The Placeholder Trick)
    preset_placeholder = st.empty()
    # 3. Draw Map First (Allows map clicks to process before dropdowns are drawn)
    render_interactive_map(keys, form_key, state_area_lookup)

    # 4. Fill the Top Dropdown
    ...
    # =====================================================================
    # 5. RENDER FEATURE INPUTS (UI Form Fields)
    # =====================================================================
    st.subheader("Property Features")
    col1, col2 = st.columns(2)

    with col1:
        current_state = st.session_state.get(keys["state"], "Selangor")
        state_idx = available_states.index(current_state) if current_state in available_states else 0

        state = st.selectbox(
            "State", options=available_states, index=state_idx,
            key=keys["state"], on_change=handle_state_change, args=(keys, state_area_lookup)
        )
        ...
        current_area = st.session_state.get(keys["area"], "")
        if current_area not in areas_in_state:
            current_area = areas_in_state[0]
            st.session_state[keys["area"]] = current_area
        ...
        current_tenure = st.session_state.get(keys["tenure"], "Freehold")
    ...
    with col2:
        ...
        current_type = st.session_state.get(keys["type"], "Terrace House")
        ...
        if use_default_tx:
            manual_tx = st.number_input(
                ...
                help="Locked to historical median."
            )
        else:
